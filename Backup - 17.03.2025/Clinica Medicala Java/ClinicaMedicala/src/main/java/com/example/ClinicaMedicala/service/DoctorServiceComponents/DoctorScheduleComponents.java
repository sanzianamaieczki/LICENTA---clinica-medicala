...
public class DoctorScheduleComponents {
    ...
    public List<DoctorScheduleDTO> getDoctorScheduleByDoctor(
            int id_doctor,
            Boolean is_deleted,
            String day_of_week,
            Time start_time,
            Time end_time) {
        return doctorScheduleRepository.findDoctorScheduleByDoctorId(id_doctor, is_deleted, day_of_week, start_time, end_time).stream()
                .map(DoctorScheduleDTO::new)
                .collect(Collectors.toList());
    }
